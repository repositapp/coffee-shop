window.Products = (() => {
 ...
 const getById = id => STORE_DATA.products.find(product => product.id === id || product.slug === id);
 const related = product => STORE_DATA.products.filter(item => item.category === product.category && item.id !== product.id).slice(0, 4);
 ...
 document.querySelectorAll('[data-category-filter]').forEach(btn => btn.addEventListener('click', () => { state.category = btn.dataset.categoryFilter; state.page = 1; renderProductsPage(); }));
 document.getElementById('product-search')?.addEventListener('input', Utils.debounce(event => { state.query = event.target.value; state.page = 1; renderProductsPage(false); }, 180));
 document.getElementById('min-price')?.addEventListener('input', event => { state.minPrice = event.target.value; state.page = 1; renderProductsPage(false); });
 document.getElementById('max-price')?.addEventListener('input', event => { state.maxPrice = event.target.value; state.page = 1; renderProductsPage(false); });
 document.getElementById('sort-product')?.addEventListener('change', event => { state.sort = event.target.value; renderProductsPage(false); });
 document.querySelectorAll('[data-view-mode]').forEach(btn => btn.addEventListener('click', () => { state.view = btn.dataset.viewMode; renderProductsPage(false); }));
 };
 ...
 const current = list.slice((state.page - 1) * state.perPage, state.page * state.perPage);
 const grid = document.getElementById('products-grid');
 const info = document.getElementById('products-info');
 const pagination = document.getElementById('products-pagination');
 if (!grid) return;
 grid.className = state.view === 'grid' ? 'grid sm:grid-cols-2 xl:grid-cols-3 2xl:grid-cols-4 gap-5' : 'grid gap-4';
 grid.innerHTML = current.length ? current.map(product => card(product, state.view)).join('') : `<div class="soft-card rounded-3xl p-8 text-center col-span-full"><i class="fa-solid fa-magnifying-glass text-4xl text-slate-300 mb-4"></i><h3 class="font-display text-2xl font-extrabold">Produk tidak ditemukan</h3><p class="text-slate-500 mt-2">Ubah kata kunci atau filter produk.</p></div>`;
 if (info) info.textContent = `${list.length} produk ditemukan`;
 if (pagination) pagination.innerHTML = Array.from({ length: totalPages }, (_, i) => `<button onclick="Products.goPage(${i + 1})" class="w-10 h-10 rounded-full ${state.page === i + 1 ? 'bg-coffee-700 text-white' : 'bg-slate-100 '} font-bold">${i + 1}</button>`).join('');
 };

 const goPage = page => { state.page = page; renderProductsOnly(); window.scrollTo({ top: 0, behavior: 'smooth' }); };
 ...
 const best = getAll().filter(p => p.isBestSeller).slice(0, 4);
 const latest = getAll().filter(p => p.isNew).slice(0, 4);
 const flash = getAll().filter(p => p.isFlashSale).slice(0, 4);
 root.innerHTML = `
 ...
 ${productRail('Menu Coffee', 'Coffee', getAll().filter(p => p.category === 'coffee').slice(0, 4), 'coffee')}
 ${productRail('Menu Non Coffee', 'Non Coffee', getAll().filter(p => p.category === 'non-coffee').slice(0, 4), 'non-coffee')}
 ${productRail('Menu Food', 'Food', getAll().filter(p => p.category === 'food').slice(0, 4), 'food')}
 <section class="container-page py-8 md:py-12" data-animate><div class="grid md:grid-cols-4 gap-5">${[['fa-truck-fast','Cepat','Delivery 30-60 menit'],['fa-shield-heart','Aman','Pembayaran simulasi aman'],['fa-leaf','Fresh','Bahan dipilih harian'],['fa-headset','Support','Layanan pelanggan responsif']].map(i => `<div class="soft-card rounded-3xl p-6"><i class="fa-solid ${i[0]} text-3xl text-orange-600"></i><h3 class="font-display text-xl font-extrabold mt-4">${i[1]}</h3><p class="text-sm text-slate-500 mt-2">${i[2]}</p></div>`).join('')}</div></section>
 ...
 return { getAll, getById, related, card, quickView, renderProductsPage, renderHome, goPage, priceBlock };
})();
