window.UI = (() => {
  ...
  const countCart = () => Storage.getCart().reduce((sum, item) => sum + Number(item.quantity || 0), 0);
  const countWishlist = () => Storage.getWishlist().length;

  const categoryLabel = category => ({ coffee: 'Coffee', 'non-coffee': 'Non Coffee', food: 'Food' }[category] || '');
  ...
  const activeMenuLabel = () => {
    ...
    if (current === 'product-detail.html' || current === 'product.html') {
      ...
      const product = STORE_DATA.products.find(item => item.id === id || item.slug === id);
      ...
      <header class="site-navbar fixed top-0 inset-x-0 z-[90] border-b border-[#E7D9C8]/80 bg-[#F8F5F2]/92 backdrop-blur-xl transition-all duration-300">
        ...
        <nav class="container-page min-h-[76px] py-3 flex items-center justify-between gap-4" aria-label="Navigasi utama">
          <a href="index.html" class="flex items-center gap-3 shrink-0" aria-label="Coffee Shop Store Home">
            ...
            <span class="font-display font-extrabold text-lg tracking-tight text-[#3B281D] leading-tight">Coffee<span class="text-orange-600">Store</span></span>
          </a>
    ...
    root.innerHTML = `
      <footer class="mt-16 bg-[#F1E7DC] text-[#4E3727] relative overflow-hidden border-t border-[#E3D0BF]">
        ...
        <div class="container-page relative py-12 md:py-16 grid md:grid-cols-2 lg:grid-cols-4 gap-8">
          <div>
            <div class="flex items-center gap-3 text-[#3B281D]"><span class="w-11 h-11 rounded-2xl bg-gradient-to-br from-coffee-700 to-orange-600 text-white flex items-center justify-center"><i class="fa-solid fa-mug-hot"></i></span><strong class="font-display text-xl">CoffeeStore</strong></div>
            <p class="mt-4 text-sm leading-7 text-[#7B543C]">Toko online coffee, non coffee, dan food dengan simulasi frontend modern berbasis Local Storage.</p>
          ...
          <div><h3 class="font-display font-extrabold text-[#3B281D] mb-4">Newsletter</h3><p class="text-sm text-[#7B543C] mb-4">Dapatkan info promo dan menu baru.</p><div class="flex gap-2"><input class="w-full rounded-full bg-white/80 border border-[#E3D0BF] px-4 py-3 outline-none" placeholder="Email kamu"><button onclick="Toast.success('Newsletter berhasil disimpan')" class="rounded-full bg-orange-600 px-4 font-bold text-white">Kirim</button></div><div class="mt-4 rounded-3xl bg-white/70 border border-[#E3D0BF] h-32 flex items-center justify-center text-sm"><i class="fa-solid fa-map-location-dot mr-2"></i> Maps Dummy</div></div>
        </div>
        <div class="relative border-t border-[#E3D0BF] py-5 text-center text-sm text-[#7B543C]">© 2026 CoffeeStore. All rights reserved.</div>
      </footer>`;
  };

  const renderFloating = () => {
    const root = document.getElementById('floating-root') || document.createElement('div');
    ...
    window.addEventListener('scroll', () => back?.classList.toggle('hidden', window.scrollY < 600), { passive: true });
  ...
  const updateBadges = () => {
    ...
    document.querySelectorAll('[data-wishlist-count]').forEach(el => el.textContent = countWishlist());
  };

  const skeletonCards = (count = 8) => Array.from({ length: count }, () => `<div class="rounded-3xl overflow-hidden soft-card p-4"><div class="skeleton h-48 rounded-2xl"></div><div class="skeleton h-4 rounded mt-4 w-2/3"></div><div class="skeleton h-4 rounded mt-3 w-1/2"></div><div class="skeleton h-10 rounded-full mt-5"></div></div>`).join('');
  ...
  return { init, renderNavbar, renderFooter, renderFloating, updateBadges, skeletonCards };
})();
